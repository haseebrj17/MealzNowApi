    ...
    public class FranchiseApi
    {
        ...
        public async Task<HttpResponseData> UpdateOrderStatus([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequestData req)
        {
            ...
            if (request.OrderStatus == null)
                return req.CreateResponse(HttpStatusCode.BadRequest);
            var data = await _franchiseService.UpdateOrderStatus(request.OrderId.Value, request.DayId.Value, request.ProductByTimingId.Value, Enum.Parse<OrderStatus>(request.OrderStatus.Value.ToString()), loggedInUser.FranchiseId.Value);
            var response = req.CreateResponse(HttpStatusCode.OK);
            await response.WriteAsJsonAsync(data);
            return response;
        }
